...
class TestTrim:
    ...
    def test_artificial_minus_one(self):
        ...
        # Original model to remove things from
        initial_model = relative_location(
            os.path.join("assets", "artificial-mechanism.yaml")
        )
        ...
        for sp in exclusion_list:
            assert all(
                [
                    sp not in {**rxn.reactants, **rxn.products}
    ...
    def testArtRemoveAll(self):
        ...
        with pytest.raises(ValueError):
            trim(initial_model, exclusion_list, "a-m.yaml")
    ...
    def test_remove_explicit_third_bodies(self):
        ...
        initial_model = relative_location(
            os.path.join("assets", "model-third-bodies.yaml")
        )
