...
def mod_dijkstra(
    G, source, get_weight, pred=None, paths=None, cutoff=None, target=None
):
    ...
    while fringe:
        ...
        for u, e in G_succ[
            v
        ].items():  # For all adjancent edges, get weights and multiply them by current path taken.
            ...
            #'weights greater than one?')
                ...
                u not in seen or vu_dist > seen[u]
...
def create_drgep_matrix(state, solution):
    ...
    flags = np.where(
        (
            (solution.product_stoich_coeffs != 0)
...
def reduce_drgep(
    ...
    if (
        previous_model
        and len(species_removed) == solution.n_species - previous_model.model.n_species
    ):
    ...
    reduced_model_filename = soln2yaml.write(
...
def run_drgep(
    ...
    if reduced_model.error > error_limit:
        ...
        soln2yaml.write(
            reduced_model, f"reduced_{reduced_model.model.n_species}.yaml", path=path
        )
