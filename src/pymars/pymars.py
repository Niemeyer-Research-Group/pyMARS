...
def parse_inputs(input_dict):
    ...
    return ReductionInputs(
        ...
        psr_conditions=psr_inputs,
        flame_conditions=flame_inputs,
        method=method,
...
def main(
    ...
    if run_sensitivity_analysis:
        ...
        reduced_model = run_sa(
            ...
            phase_name=phase_name,
            algorithm_type=sensitivity_type,
...
def pymars(argv):
    ...
    parser.add_argument(
        "-i", "--input", help="YAML file with reduction inputs", type=str
    )

    parser.add_argument(
        "--path", help="Path to directory for writing files.", type=str, default=""
    )
    parser.add_argument(
        "--num_threads",
        help=(
    ...
    parser.add_argument(
        "--convert",
        help="Convert files between Cantera and Chemkin formats (.yaml <=> .inp)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    if args.version:
        ...
        print(
            "pyMARS {version} from {path} ()".format(
    ...
    if args.convert:
        ...
        if os.path.splitext(inputs.model)[1] not in (".yaml", ".yml"):
        ...
        main(
            ...
            inputs.psr_conditions,
            inputs.flame_conditions,
