...
- Added perfectly stirred reactor (PSR) support via the `psr-conditions` input block, reproducing the original Fortran/Chemkin MARS capability (#100). A new `PSRSimulation` traces the steady temperature-vs-residence-time response curve through its extinction turning point using pseudo-arclength continuation (`psr_solver`, built on Cantera kinetics derivatives and `scipy`), and samples three points from the burning branch: the extinction turning point, the point nearest 0.1 s, and their log-midpoint. The error metric is the larger of the extinction residence-time error and the response-temperature errors. Requires `scipy`.
