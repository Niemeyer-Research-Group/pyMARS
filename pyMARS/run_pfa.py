...
import soln2ck
import soln2cti
from get_rate_data_pfa import get_rates_pfa
from pfa_loop_control import pfa_loop_control
from autoignition_loop_control import autoignition_loop_control
import numpy as np

def run_pfa(args, solution_object,error,past):           
	"""
    Function to run the pfa method for model reduction
	...
	rate_edge_data = get_rates_pfa('mass_fractions.hdf5', solution_object) #Get edge weight calculation data. 
	
	print("Testing for starting threshold value")
	pfa_loop_control(solution_object, args, error, threshold, done, rate_edge_data) #Trim the solution at that threshold and find the error.
	while error[0] != 0: #While the error for trimming with that threshold value is greater than allowed.
		...
		n = n + 1
		pfa_loop_control(solution_object, args, error, threshold, done, rate_edge_data)
		if error[0] <= .02:
			error[0] = 0
	...
	while not done[0] and error[0] < args.error: #Run the simulation until nothing else can be cut. 
		sol_new = pfa_loop_control( solution_object, args, error, threshold, done, rate_edge_data) #Trim at this threshold value and calculate error.
	...
	print("\nGreatest result: ")
	sol_new = pfa_loop_control( solution_object, args, error, max_t, done, rate_edge_data)
