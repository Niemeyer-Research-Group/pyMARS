...
def trim_drg(total_edge_data, solution_object, threshold_value, keeper_list, done, target_species):
    ...
    for ic in total_edge_data.keys():  # For each initial condition
        ...
        for tstep in total_edge_data[ic].keys():
            ...
            # Each species
            for species_a, denom in enumerate(denominator):
                try:
                    species_a_name = solution_object.species_names[species_a]
                    if denom != 0:
                        connected = np.where(numerator[species_a, :] != 0)[0]
                        for species_b in connected:
                            species_b_name = solution_object.species_names[species_b]
                            # DRG weight between two species
                            if denominator[species_a] != 0:
                                weight = abs(
                                    float(numerator[species_a, species_b]) / float(denominator[species_a]))
...
def get_rates_drg(sim_array, solution_object):
    ...
    for ic in sim_array:
        ...
        for tstep in ic.sample_points:  # Iterate through all timesteps
            ...
            denom = np.zeros(new_solution.n_species)
            numerator = np.zeros((new_solution.n_species, new_solution.n_species))
            reac_prod_rate = new_reaction_production_rates
            reactants = new_solution.reactant_stoich_coeffs()
            products = new_solution.product_stoich_coeffs()
            net_stoich = products - reactants
            abs_stoich = np.abs(products) + np.abs(reactants)
            valid = np.where(reac_prod_rate != 0)[0]
            if valid.size:
                base_rates = np.abs(
                    net_stoich[:, valid] * new_reaction_production_rates[valid])
                denom = np.sum(base_rates, axis=1)
                for sp_b in range(new_solution.n_species):
                    numerator[:, sp_b] += np.sum(
                        base_rates[:, np.where(abs_stoich[sp_b, valid])[0]], axis=1)
